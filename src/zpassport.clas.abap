CLASS zpassport DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS build_passport_table
      IMPORTING
                file             TYPE string
      RETURNING VALUE(passports) TYPE string_table.
    METHODS passport_valid
      IMPORTING
...
CLASS zpassport IMPLEMENTATION.
  ...
  METHOD count_valid.
    ...
    valid_passports = REDUCE #( INIT x = 0
                                FOR passport IN passports NEXT
                                x += COND #( WHEN passport_valid( passport && ` ` ) "add space at the end for pattern check of last item in line
                                             THEN 1
                                             ELSE 0 ) ).
  ENDMETHOD.
  ...
  METHOD build_passport_table.
    ...
    LOOP AT file_w_empty ASSIGNING FIELD-SYMBOL(<line>).
      IF <line> IS NOT INITIAL.
        passports[ row ] = |{ passports[ row ] } {  <line> }|.
      ELSE.
        row += 1.
        APPEND INITIAL LINE TO passports.
      ENDIF.
  ...
  METHOD passport_valid.
    "byr (Birth Year)
    CHECK find( val = passport sub = |byr:| ) > -1
      AND check_byr( year = CONV #( substring_before( val = substring_after( val = passport sub = 'byr:' ) sub = ` ` ) ) ).

    "iyr (Issue Year)
    CHECK find( val = passport sub = |iyr:| ) > -1
      AND check_iyr( year = CONV #( substring_before( val = substring_after( val = passport sub = 'iyr:' ) sub = ` ` ) ) ).

    "eyr (Expiration Year)
    CHECK find( val = passport sub = |eyr:| ) > -1
      AND check_eyr( year = CONV #( substring_before( val = substring_after( val = passport sub = 'eyr:' ) sub = ` ` ) ) ).

    "hgt (Height)
    CHECK find( val = passport sub = |hgt:| ) > -1
      AND check_hgt( height = substring_before( val = substring_after( val = passport sub = 'hgt:' ) sub = ` ` ) ).

    "hcl (Hair Color)
    CHECK find( val = passport sub = |hcl:| ) > -1
      AND check_hcl( hcl = substring_before( val = substring_after( val = passport sub = 'hcl:' ) sub = ` ` ) ).

    "ecl (Eye Color)
    CHECK find( val = passport sub = |ecl:| ) > -1
      AND check_ecl( ecl = substring_before( val = substring_after( val = passport sub = 'ecl:' ) sub = ` ` ) ).

    "pid (Passport ID)
    CHECK find( val = passport sub = |pid:| ) > -1
      AND check_pid( pid = substring_before( val = substring_after( val = passport sub = 'pid:' ) sub = ` ` ) ).
